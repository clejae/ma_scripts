...
folder_lst = glob.glob(r'Y:\germany-drought\VPI_Copernicus\*')
...
for folder in folder_lst:
    ...
    for in_pth in file_lst:
        ...
        try:
            start = info.index("VPI_SCALING_FACTOR=") + len("VPI_SCALING_FACTOR=")  # index of last character if this string
            end = info.index("\n", start)  # index of first appearance of given character after start index
...
for folder in folder_lst:
    ...
    file_lst = glob.glob(folder + '\g2_BIOPAR_VPI_2*.tif')

    file_name = file_lst[0][:-22]

    print("Length", len(file_lst))

    vrt = gdal.BuildVRT(file_name + '.vrt', file_lst)

    #outputSRS
    #options = gdal.TranslateOptions(outputSRS='EPSG:3035')
    ds = gdal.Translate(file_name + '.tif', vrt)
    del(ds)
    del(vrt)

    os.remove(file_name + '.vrt')
...
for folder in folder_lst:
    ...
    file_lst = glob.glob(folder + '\g2_BIOPAR_VPI_2*0.tif')
...
for folder in folder_lst:
    ...
    file_lst = glob.glob(folder + '\g2_BIOPAR_VPI_2*5.tif')

    #file= file_lst[0]

    for file in file_lst:
        print(file)
        os.rename(file, r'Y:\germany-drought\VPI_Copernicus\\' + file[-35:])
...
for year_int in range(2017,2019):
    ...
    for month_str in month_str_lst:
        ...
        ras_pth_lst = glob.glob(r'Y:\germany-drought\VPI_Copernicus\g2_BIOPAR_VPI_' + year_str + month_str +'*.tif')
        ...
        for ras_pth in ras_pth_lst:
            ...
            geom_arr[geom_arr == no_data_value] = np.nan
            arr_lst.append(geom_arr)
        ...
        output_name = r'Y:\germany-drought\VPI_Copernicus\g2_BIOPAR_VPI_' + year_str + month_str +'.tif'
        ...
        target_ds.SetGeoTransform((x_min_ext, gt[1], 0, y_max_ext, 0, gt[5]))
