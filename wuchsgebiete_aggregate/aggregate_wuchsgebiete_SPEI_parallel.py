...
for spei in [3,6,12,24]:
    ...
    out_pth = r'O:\Student_Data\CJaenicke\00_MA\data\vector\lsp_metrics\wuchsgebiete\SPEI{0:02d}_2018_wuchsgebiete_v2.csv'.format(spei)

    wuchs_shp_outer = ogr.Open(wuchs_pth)
    wuchs_lyr_outer = wuchs_shp_outer.GetLayer()

    # if spei == 3 or spei == 6 or spei == 12:
    #     no_data_val = 32767
    # if spei == 24:
    no_data_val = -32767
    ...
    for i in range(feat_count):
        ...
        wuchs_id = feat.GetField('ID')
        print(spei, wuchs_id, wuchs_name)
        ...
        file_lst = [
            r'O:\Student_Data\CJaenicke\00_MA\data\climate\SPEI\tiles\{0}\SPEI{1:02d}_2018.tif'.format(t, spei)
            for t in tiles_lst]
        vrt_ind_pth = r'O:\Student_Data\CJaenicke\00_MA\data\vrt\wuchsgebiete\SPEI\{1}_{0}_SPEI{2:02d}_2018.vrt'.format(
            wuchs_name,wuchs_id,spei)
        ...
        print(spei, wuchs_id, wuchs_name, 'done.')
